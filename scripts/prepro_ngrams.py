...
def main(params):
  ...
  wtoi = {w:i for i,w in list(itow.items())}
  ...
  with open(params['output_pkl']+'-words.p','wb') as f:
    cPickle.dump({'document_frequency': ngram_words, 'ref_len': ref_len}, f, protocol=cPickle.HIGHEST_PROTOCOL)
  with open(params['output_pkl']+'-idxs.p','wb') as f:
    cPickle.dump({'document_frequency': ngram_idxs, 'ref_len': ref_len}, f, protocol=cPickle.HIGHEST_PROTOCOL)
